...
public class RoundRobinTaskScheduler extends TaskScheduler {
  ...
  private HashMap<String, Queue<TaskReservation>> userQueues = Maps.newHashMap();
  ...
  synchronized int handleSubmitTaskReservation(TaskReservation taskReservation) {
    ...
    if (!userQueues.containsKey(user)) {
      users.add(user);
      userQueues.put(user, new LinkedList<TaskReservation>());
    }
    ...
    Queue<TaskReservation> reservations = userQueues.get(user);
  ...
  protected synchronized void handleTaskCompleted(
    ...
    if (numQueuedReservations != 0) {
      ...
      for (int offset = 0; offset < users.size(); offset++) {
        ...
        Queue<TaskReservation> considering = userQueues.get(user);
